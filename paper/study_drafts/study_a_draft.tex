...
\begin{abstract}
\noindent This is an internal, numbers-focused draft covering all Study A results to date. Study A trains an ImageNet-pretrained DenseNet-121 to reproduce Larrazabal et al.\ (2020)'s sex-imbalance AUC gap on NIH ChestX-ray14 pneumothorax classification, then stress-tests that gap across training seeds, patient splits, and training-set sizes before it is handed off, frozen, to Study B. The direction of the test oracle passes and is robust across every check performed. The magnitude does not: compared against the nearest points on Larrazabal et al.'s Figure 1, the reproduced 90/10 gap is roughly half to a third of theirs, outside the originally planned 2-AUC-point tolerance. Section~\ref{sec:magnitude} reports this comparison and the reasoning for treating it as a documented limitation rather than a blocking failure. Separately, the magnitude ordering among the 70/30 and 50/50 arms is noisier than a single canonical run suggests, and is at least partly explained by (a) a small-sample evaluation noise floor and (b) a fixed total training budget that couples the imbalance ratio to absolute subgroup training-set size. No narrative analysis beyond what is needed to interpret the numbers is included here; this is a results record, not a paper section.
\end{abstract}
...
\textbf{Test oracle.} Reproduce Larrazabal et al.\ (2020)'s subgroup AUC gap (majority AUC $-$ minority AUC) in the same direction, on the 90/10 arm, canonical seed 42, canonical split. Larrazabal et al.\ report Pneumothorax only as box plots with no 90/10 point and no numeric table, so only the \emph{direction} half of the oracle is automated; the magnitude half was checked manually against their Figure 1 (Section~\ref{sec:magnitude}) and is now a documented comparison rather than a strict pass/fail gate.
...
Cross-split (canonical + splits 101--103) & 0.0670 & 0.0716 & [0.0657, 0.0815] & True (4/4) \\
\bottomrule
\end{tabular}
\caption{90/10 arm robustness checks: varying training stochasticity (cross-seed) and varying which patients land in test (cross-split). Both non-gating. Following \texttt{\_gap\_spread} in \texttt{src/metrics.py}, the Mean, Range, and direction-agreement columns are computed over the canonical run \emph{plus} the replicates in each row, so the cross-seed row covers 5 runs (seed 42 is the canonical one) and the cross-split row covers 4 (canonical split + splits 101--103). The mean over the 3 alternate splits alone would be 0.0731.}
...
\subsection{Magnitude comparison against Larrazabal et al.'s Figure 1}
\label{sec:magnitude}

Larrazabal et al.\ report Pneumothorax only as box plots (Fig.\ 1, panels B-2/C-2: a single model trained on a mixed-sex ratio, evaluated separately on male and female test folds — the panel methodologically comparable to this study's gap definition), with training compositions at 0/25/50/75/100\% female and no 90/10 point. Values below are a visual read off the box-plot mean markers (PDF rendered locally; not a pixel-calibrated digitization), treated as $\pm0.01$.
...
10\% (interpolated) & --- & --- & $\approx$0.12 \\
\bottomrule
\end{tabular}
\caption{Larrazabal et al., Fig.\ 1 panels B-2/C-2, Pneumothorax, read at the two grid points nearest Study A's 90/10 (10\% female) composition, plus a linear interpolation to 10\%.}
\label{tab:larrazabal}
\end{table}

Study A's canonical 90/10 gap is 0.0670 (mean across 5 seeds: 0.0535). Against the interpolated Larrazabal value ($\approx$0.12), the discrepancy is $\approx$0.05; against the two raw neighboring points (0.135, 0.10) it is 0.03--0.08. All of these exceed the originally planned 2-AUC-point (0.02) tolerance by 1.5--4$\times$. Direction matches in every source; magnitude does not, on a literal reading of that tolerance.

Two structural reasons this is reported as a documented limitation rather than a blocking failure, not as an attempt to explain the gap away: (1) no exact 90/10 point exists on Larrazabal's grid, so any comparison already requires interpolation or a neighboring-point proxy that the original tolerance did not anticipate; (2) the two studies estimate the gap with different estimators — Larrazabal's box plots aggregate 20 folds, while this study's spread comes from 5 training seeds and 3 alternate splits on one fixed configuration — and a tolerance calibrated for an exact same-estimator comparison does not automatically transfer across estimation protocols. Study B's actual dependency on Study A is a real, correctly-signed, statistically significant subgroup gap (bootstrap CI excludes zero, Table~\ref{tab:canonical}), which this magnitude discrepancy does not undermine; it is not a claim that this pipeline exactly reproduces Larrazabal et al.'s reported magnitude.
...
\textbf{Oracle status.} The direction half of the test oracle passes and is robust: majority AUC exceeds minority AUC in every one of the 23 independent training runs reported here — 11 at the 90/10 ratio (5 training seeds incl.\ canonical, 3 alternate splits, 3 at $N=5{,}000$) and 6 at each of the 70/30 and 50/50 ratios (3 canonical-$N$ seeds + 3 at $N=5{,}000$) — with zero direction reversals in any of them. The magnitude half was checked manually against Larrazabal et al.'s Figure 1 (Section~\ref{sec:magnitude}) and does not pass the originally planned 2-AUC-point tolerance — the reproduced gap is roughly half to a third of the Larrazabal-equivalent value at the nearest comparable training compositions. This is reported as an honest limitation of the reproduction, not concealed: direction transfers cleanly to this pipeline, magnitude does not, and the two studies' different variance-estimation protocols (20-fold aggregation vs.\ this study's seed/split replication) mean the discrepancy is not fully resolved by this analysis alone.
...
\section{Limitations}
\begin{itemize}
\item The test oracle's magnitude criterion is not automated (no numeric Larrazabal target exists for the 90/10 condition) and, on the manual comparison performed (Section~\ref{sec:magnitude}), does not pass the originally planned 2-AUC-point tolerance — the reproduced 90/10 gap is $\approx$0.03--0.08 below the nearest Larrazabal reference points. This is a genuine limitation of the reproduction, not fully resolved by the noise-floor and fixed-budget explanations offered elsewhere in this draft for the 70/30-vs-50/50 ordering.
\item The Figure 1 values in Section~\ref{sec:magnitude} are a visual read of box-plot mean markers, not a pixel-calibrated digitization, and carry $\pm0.01$ uncertainty on top of the discrepancy itself.
